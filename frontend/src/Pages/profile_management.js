  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        />
        <h1 className="text-2xl font-bold text-center flex-1">Profile Management</h1>
      </div>

      <div className="border border-gray-300 rounded-2xl p-6 shadow-lg space-y-6">
        <div className="grid grid-cols-2 gap-6">
          {['profile', 'cover'].map((type) => (
            <div key={type} className="space-y-4">
              <label className="block mb-2 font-semibold capitalize">{type} picture</label>
              <motion.div
                whileHover={{ scale: 1.05 }}
                className="relative border border-gray-300 rounded-2xl p-3 shadow-lg"
              >
                {images[type] ? (
                  ...
                  <label className="cursor-pointer">
                    ...
                    />
                    <div className="w-full h-48 flex items-center justify-center border-2 border-dashed border-gray-300 rounded-2xl">
                      Upload {type} picture
                    </div>
                  </label>
                )}
      ...
      <hr className="border-t border-gray-300 mt-6 w-full" />
      <Post/>
    </div>
