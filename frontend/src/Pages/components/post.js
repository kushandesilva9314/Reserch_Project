...
const PostPage = () => {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get("http://localhost:3001/api/posts/user", {
  ...
  {posts.length > 0 ? (
    posts.map((post) => (
      <tr key={post._id} className="border-b border-gray-300 hover:bg-gray-100 transition">
        <td className="p-2 text-black">{post.title}</td>
        <td className="p-2 flex justify-end space-x-2">
          <button
            className="bg-green-500 hover:bg-green-400 text-white p-2 rounded-lg"
    ...
    <tr>
      <td colSpan="2" className="p-4 text-center text-black">No posts found</td>
    </tr>
  )}
</tbody>
